...
juce::PopupMenu Grace::GraceMenuBarModel::getMenuForIndex(int idx, const juce::String &name)
{
  ...
  else if (name == "Audio")
  {
    ...
    menu.addSeparator();
    // Osc
    if (OpenSoundControl::getInstance()->isAvailable())
    {
      juce::String t ("OSC Out: ");
      juce::PopupMenu s;
      if (int p = OpenSoundControl::getInstance()->getConnectedPort(true))
      {
        t << "port " << juce::String(p);
        s.addCommandItem(m, CommandIDs::OscTraceOutput);
        menu.addSubMenu(t, s);
      }
      else
      {
      ...
      s.clear();
      t = "OSC In: ";
      if (int p = OpenSoundControl::getInstance()->getConnectedPort(false))
      {
        t << "port " << juce::String(p);
...
void Grace::getCommandInfo(const juce::CommandID id, juce::ApplicationCommandInfo& info)
{
  ...
  case CommandIDs::FileClearInit:
    ...
    info.setActive(pref->getStringProp("LispInitFile") != juce::String::empty);
    break;
  ...
  case CommandIDs::OscTraceInput:
    ...
    info.setTicked(OpenSoundControl::getInstance()->isTracingActive(false));
    break;
  case CommandIDs::OscTraceOutput:
    info.shortName = "Trace Output";
    info.setTicked(OpenSoundControl::getInstance()->isTracingActive(true));
    break;
  ...
  case CommandIDs::AudioSynthSettings:
    ...
    info.setActive(AudioManager::getInstance()->internalSynth ? true : false);
    break;
...
bool Grace::perform(const juce::ApplicationCommandTarget::InvocationInfo& info)
{
  ...
  case CommandIDs::FileSaveVersion:
    ...
    break;
  case CommandIDs::FilePlayAudioFile:
    AudioFilePlayer::openAudioFilePlayer(juce::File::nonexistent, false);
    break;
  case CommandIDs::FilePlayMidiFile:
    MidiFilePlayer::openMidiFilePlayer(juce::File::nonexistent);
    break;
  case CommandIDs::FileLoad:
    SchemeThread::getInstance()->load(juce::File::nonexistent,true);	
    break;
  case CommandIDs::FileLoadRecent:
  ...
  case CommandIDs::FileSetInit:
    ...
    break;
  case CommandIDs::FileClearInit:
    Preferences::getInstance()->setStringProp("LispInitFile", juce::String::empty);
    break;
  case CommandIDs::FileOpenInit:
  ...
  case CommandIDs::EditDelete:
    if (CodeEditorWindow* w = dynamic_cast<CodeEditorWindow*>(activeWindow))
      w->getCodeEditor()->insertTextAtCaret(juce::String::empty);
    break;
  case CommandIDs::EditSelectAll:
    if (CodeEditorWindow* w = dynamic_cast<CodeEditorWindow*>(activeWindow))
  ...
  case CommandIDs::OscTraceInput:
    {
      bool a = OpenSoundControl::getInstance()->isTracingActive(false);
      OpenSoundControl::getInstance()->setTracingActive(!a, false);
    }
    break;
  case CommandIDs::OscTraceOutput:
    {
      bool a = OpenSoundControl::getInstance()->isTracingActive(true);
      OpenSoundControl::getInstance()->setTracingActive(!a, true);
    }
    break;
