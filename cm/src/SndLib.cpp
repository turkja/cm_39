...
bool SchemeThread::init()
{
  ...
  juce::String str=juce::String::empty;
...
juce::String SndLib::getInstrumentCode(juce::String filename)
{
...
{
  juce::XmlElement* insts = getInstrumentTableInstruments();
  juce::String s = juce::String::empty;
  forEachXmlChildElement (*insts, e)
  {
...
void SndLib::autoLoadInstruments()
{
  ...
  {
    juce::String str = getInstrumentCode(loads[i]);
...
void InstrumentTable::buttonClicked (juce::Button* button)
{
  ...
  else if (button == editInstButton)
  {
    ...
    if (code.isEmpty())
      return;
    CodeEditorWindow::newFile(juce::File(path).getFileName(), TextIDs::Lisp, code);
  } 
