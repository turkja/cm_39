  ...
  for (int i = schemethread->schemeNodes.size() - 1; i >= 0; i--)
    if ((n = dynamic_cast<XProcessNode*>(schemethread->schemeNodes[i])))
      if (n->userid == userid)
      {
        if(firstNode)
        {
          s7_gc_unprotect(sc, n->schemeproc);
          n->schemeproc = schemeproc;
          s7_gc_protect(sc, n->schemeproc);
          firstNode = false;
        }
        else
...
XOscNode::XOscNode(double qTime, juce::String oscPath, juce::String oscTypes)
  : XSchemeNode(qTime),
    OpenSoundControl::OscMessageData(oscPath, oscTypes),
    hook (0)
{
}
...
OscHook* SchemeThread::getOscHook(juce::String path, bool strict)
{
  ...
  for (int i = 0; i < oscHooks.size() && !hook; i++)
  {
    ...
    // hook matches oper if its the same or default hook set
    if (h->path == path || (h->path==juce::String::empty && !strict))
      hook = h;
  }
  return hook;
}
...
bool XOscNode::applyNode(SchemeThread* st, double curtime)
{
  ...
  // initialize data to (#f) for fast nconc'ing onto the end
  s7_pointer data = s7_cons(sc, st->schemeFalse, snil);
  ...
  for (int i = 0; i < oscTypes.length(); i++)
  {
    ...
    case OpenSoundControl::OSC_INT64:
      s7_set_cdr(tail, s7_cons(sc, s7_make_integer(sc, oscInts[I++]), snil));
      break;
    case OpenSoundControl::OSC_FLOAT:
    case OpenSoundControl::OSC_DOUBLE:
    case OpenSoundControl::OSC_TIMETAG:
      s7_set_cdr(tail, s7_cons(sc, s7_make_real(sc, oscFloats[F++]), snil));
      break;
    case OpenSoundControl::OSC_STRING:
      s7_set_cdr(tail, s7_cons(sc, s7_make_string(sc, oscStrings[S++].toUTF8()), snil));
      break;
    case OpenSoundControl::OSC_SYMBOL:
      s7_set_cdr(tail, s7_cons(sc, s7_make_symbol(sc, oscStrings[S++].toUTF8()), snil));
      break;
    ...
    case OpenSoundControl::OSC_CHAR:
      s7_set_cdr(tail, s7_cons(sc, s7_make_character(sc, (char)oscInts[I++]), snil));
      break;
    case OpenSoundControl::OSC_MIDI:
      {
        I++; // skip over size value
        s7_pointer m = 
          s7_cons(sc, s7_make_integer(sc, oscInts[I + 0]),
             s7_cons(sc, s7_make_integer(sc, oscInts[I + 1]),
                s7_cons(sc, s7_make_integer(sc, oscInts[I + 2]),
                 s7_cons(sc, s7_make_integer(sc, oscInts[I + 3]), snil))));
    ...
    case OpenSoundControl::OSC_BLOB:
      {
        int l = (int)oscInts[I++]; // length of blob
        s7_pointer m = snil;
        // reverse collect for speed
        for (int j = I + l - 1; j >= I; j--)
          m = s7_cons(sc, s7_make_integer(sc, oscInts[j]), m);
        s7_set_cdr(tail, s7_cons(sc, s7_cons(sc, m, snil), snil));
        I += l;
      }
      break;
  ...
  int loc;
  s7_pointer args, res;
  // FIXME !! just use data.oscPath
  if (hook->path.isEmpty()) // "" == default hook
  {
    // default hook takes whatever path is in the message and replaces
    // the placeholder #f in car(data) with it
    s7_set_car(data, s7_make_string(sc, oscPath.toUTF8()));
