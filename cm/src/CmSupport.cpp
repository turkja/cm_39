...
juce::String cm_logo ()
{
  ...
  juce::String logo = juce::String::empty;
  logo << " /\\\\\\" << crlf
       << "---\\\\\\---------" << crlf
       << "----\\\\\\--------" << crlf
       << "----/\\\\\\-------" << " " << vers << crlf
...
void cm_load(char *path)
{
  ...
  if (file.hasFileExtension(".sal") || file.hasFileExtension(".sal2") || file.hasFileExtension(".sal1"))
  {
    sal_load(path);
  }
  else
  {
    ...
    juce::XmlElement* ins=NULL;
    if (
        // no directory component
        (! name.containsAnyOf(juce::File::separatorString))
        // file does not exist
        && (! file.existsAsFile())
        // no load path set
    ...
    {
      juce::String str=SndLib::getInstance()->getInstrumentCode(name);
...
s7_pointer cm_midifile_import(char* path, int track, s7_pointer midivalues)
{
  ...
  juce::FileInputStream* input=file.createInputStream();
...
bool sw_open_from_xml(char* text)
{
  ...
  juce::XmlElement* xml=doc.getDocumentElement();
  bool ok=true;
  if (xml)
  {
...
s7_pointer osc_open(char* port, char* targ)
{
  int inputPort = juce::String(port).getIntValue();
  juce::String target (targ);
  juce::String outputHost;
  int outputPort;

  int i = target.indexOfChar(':');
  if (i > -1)
  {
    outputHost = target.substring(0, i);
    outputPort = target.substring(i + 1).getIntValue();
  }
  else
  {
    outputHost = "localhost";
    outputPort = target.getIntValue();
  }
  // error if OSC unavailable or can't be opend
  if (!OpenSoundControl::getInstance()->openConnection(inputPort, outputHost, outputPort))
  {
    juce::String err (">>> Error: ");
    err << OpenSoundControl::getInstance()->getLastOscError() << "\n";
    SchemeThread::getInstance()->signalSchemeError(err);
  }
  return SchemeThread::getInstance()->schemeTrue;
}

s7_pointer osc_close()
{
  // error if OSC unavailable or can't be opend
  if (!OpenSoundControl::getInstance()->closeConnection())
  {
    juce::String err (">>> Error: ");
    err << OpenSoundControl::getInstance()->getLastOscError() << "\n";
    SchemeThread::getInstance()->signalSchemeError(err);
...
s7_pointer osc_is_open()
{
  if (OpenSoundControl::getInstance()->isConnected())
    return SchemeThread::getInstance()->schemeTrue;
  else
    return SchemeThread::getInstance()->schemeFalse;
}

s7_pointer osc_send_message(char* oscPath, s7_pointer list)
{
  juce::String path (oscPath);
  if (!OpenSoundControl::getInstance()->sendMessage(path, list))
  {
    juce::String err (">>> Error: osc:message failed: ");
    err << OpenSoundControl::getInstance()->getLastOscError() << "'.\n";
    SchemeThread::getInstance()->signalSchemeError(err);
  }
  return SchemeThread::getInstance()->schemeTrue;
...
s7_pointer osc_send_bundle(double time, s7_pointer list)
{
  if (!OpenSoundControl::getInstance()->sendBundle(time, list))
  {
    juce::String err (">>> Error: osc:bundle failed:");
    err << OpenSoundControl::getInstance()->getLastOscError() << "'.\n";
    SchemeThread::getInstance()->signalSchemeError(err);
  }
  return SchemeThread::getInstance()->schemeTrue;
