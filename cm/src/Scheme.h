...
public:
  juce::String path;
  s7_pointer proc;
  OscHook (juce::String oscpath, s7_pointer func) : path (oscpath), proc (func) {}
  ~OscHook () {}
};
...
class XOscNode : public XSchemeNode,
                 public OpenSoundControl::OscMessageData
{
public:
  OscHook* hook;
  XOscNode(double qtime, juce::String oscpath, juce::String osctypes = "") ;
  ~XOscNode() ;
  bool applyNode(SchemeThread* scheme, double curtime) ;
};
...
public:
  ...
  bool isOscHook(juce::String path=juce::String::empty);
