...
void AudioFilePlayer::openAudioFilePlayer(juce::File fileToOpen, bool play)
{
  ...
  static juce::File lastOpened = juce::File::nonexistent;
  ...
  if (fileToOpen == juce::File::nonexistent)
  {
...
void AudioFilePlayer::clearTransport()
{
  ...
  transport->setPlaybackRange(0.0, 0.0, false);
...
void AudioFilePlayer::loadIntoTransport(const juce::File& file)
{
  ...
  {
    audioFileReader = new juce::AudioFormatReaderSource(reader, true);
    audioFile = file;
    // NB: could buffer if we really want to play large files...
    transportSource.setSource(audioFileReader, 0, 0, reader->sampleRate); 
...
void AudioFilePlayer::positionChanged(double position, bool isPlaying)
{
  ...
  if (position == 0)
  {
    ...
    transportSource.setSource(0);
    transportSource.setSource(audioFileReader);
    if (isPlaying)
      transportSource.start();
  }
  else
